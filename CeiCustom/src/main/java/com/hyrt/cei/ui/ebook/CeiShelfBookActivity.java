...
//													shelvesview, "删除成功！");
	...
	private void initData() {
		...
		new Thread() {
			...
			public void run() {
				...
				// 按时间排序 sdfsfdsfsff
				ComparatorReport comparator = new ComparatorReport("time");
	...
	public void ShelfDownload(View arg1, final int arg2) {
		...
		if (!report.getIsLoad().equals("yes")) {
			...
			String path = report.getDownpath().toString();
			File savedir = new File(Report.SD_PATH + report.getName());// Environment.getExternalStorageDirectory().getAbsolutePath()+
																		// "/cei/bg/"
			if (!savedir.exists()) {
				savedir.mkdirs();
			}
			...
			try {
				if (loadThreads.get(report.getId()) != null) {
					/*
					 * Toast.makeText(CeiShelfBookActivity.this, "正在下载！",
					 * 2).show();
					 */
					MyTools.exitShow(CeiShelfBookActivity.this,
							CeiShelfBookActivity.this.getWindow()
	...
	public void onClick(View v) {
		if (shezhi == v) {
			...
			if (data != null && data.size() == 0)
				MyTools.exitShow(this, this.getWindow().getDecorView(),
						"没有查到您需要的信息!");
			// 横活树
			if (hengORshu == 1) {
			...
			if (loadThreads.size() > 0) {
				...
				MyTools.exitShow(CeiShelfBookActivity.this,
						CeiShelfBookActivity.this.getWindow().getDecorView(),
						"请下载完再切换视图！");
				return;
			}
			// gyh 修改背景
			// loadBackGround(R.drawable.phone_studynm_bg,
			// R.drawable.shelf_panel, 2);
			findViewById(R.id.mainlayout).setBackgroundResource(
					R.drawable.phone_studynm_bg);
	...
	public void delViewShow(final Report report, final int what) {
		// TODO Auto-generated method stub
		// 加载popupWindow的布局文件
		View contentView = LayoutInflater.from(getApplicationContext())
