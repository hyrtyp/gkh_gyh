...
public class Disclaimer extends ContainerActivity implements OnClickListener {
	...
	private TextView abouts_us,usesinfo,mazminfo;
	@Override
...
//		}
		abouts_us=(TextView)findViewById(R.id.abouts_us);
		usesinfo=(TextView)findViewById(R.id.usesinfo);
		mazminfo=(TextView)findViewById(R.id.mazminfo);
        mazminfo.setVisibility(View.GONE);
...
//					R.drawable.gywm_3_1);
//			abouts_us,usesinfo,mazminfo
			abouts_us.setTextColor(Color.WHITE);
			usesinfo.setTextColor(Color.BLUE);
			mazminfo.setTextColor(Color.BLUE);
			abouts_us.setBackgroundResource(R.drawable.phone_study_menu_select);
			usesinfo.setBackgroundResource(R.drawable.menu_transbg);
			mazminfo.setBackgroundResource(R.drawable.menu_transbg);
			if (news.size() >= 3) {
				for (int i = 0; i < news.size(); i++) {
					if (news.get(i).getTitle().endsWith(MODEL_NAME)) {
						view.loadUrl(htmlHade + news.get(i).getId());
...
//			findViewById(R.id.mazminfo).setBackgroundResource(
//					R.drawable.gywm_3_1);
			abouts_us.setTextColor(Color.BLUE);
			usesinfo.setTextColor(Color.WHITE);
			mazminfo.setTextColor(Color.BLUE);
			abouts_us.setBackgroundResource(R.drawable.menu_transbg);
			usesinfo.setBackgroundResource(R.drawable.phone_study_menu_select);
			mazminfo.setBackgroundResource(R.drawable.menu_transbg);
			if (news.size() >= 3) {
				for (int i = 0; i < news.size(); i++) {
...
//					R.drawable.gywm_3_0);
			abouts_us.setTextColor(Color.BLUE);
			usesinfo.setTextColor(Color.BLUE);
			mazminfo.setTextColor(Color.WHITE);
			abouts_us.setBackgroundResource(R.drawable.menu_transbg);
			usesinfo.setBackgroundResource(R.drawable.menu_transbg);
			mazminfo.setBackgroundResource(R.drawable.phone_study_menu_select);
			if (news.size() >= 3) {
				for (int i = 0; i < news.size(); i++) {
